...
![image](https://github.com/smart-devops-org/devops-system/assets/46982752/64c0b559-71da-4269-ab13-16267cce63a7)

- 기본적으로 파드는 클러스터의 상태를 볼 수 없는 것이 쿠버네티스 기본 정책
- 모든 사람에게 자신의 일을 하는 데 꼭 필요한 권한만 최소한으로 주고, 한 가지 이상의 권한을 주지 않는 것이 좋다.
- 각 파드를 위한 특정 서비스어카운트를 생성한 다음 롤바인딩으로 맞춤형 롤과 연계하는 것이 바람직

# 3. devops tools 정리

- cncf 리눅스 재단 소속의 비영리 단체
- 구글에서 기증한 kubernetes를 시작으로 클라우드 native 컴퓨팅 환경에서 필요한 다양한 오픈소스 프로젝트를 추진 관리
- https://landscape.cncf.io/
- 각 리소스에 대한 정리는 따로 레포지토리로 정리해서 관리 예정

## 3.1 CI/CD

- CI/CD는 Continuous Integration과 Continuous Deployment를 의미합니다. 이는 코드 변경사항을 자주 통합하고, 테스트하고, 배포하는 방법론입니다.

## 3.2 Servicemesh

- 서비스 메시는 마이크로서비스 아키텍처에서 서비스 간 통신을 관리하고, 보안, 로드 밸런싱, 실패 복구 등의 기능을 제공하는 인프라 계층입니다.
...
- 로깅은 시스템에서 발생하는 이벤트를 기록하고 저장하는 프로세스입니다. 로그는 시스템 문제를 진단하거나 보안 사고를 조사하는 데 사용됩니다.

## 3.5 Database

- 데이터베이스는 구조화된 데이터를 저장, 검색, 업데이트할 수 있는 시스템입니다.

## 3.6 Workflow

- 워크플로우는 작업이나 프로세스가 어떻게 진행되는지를 정의한 것입니다. 이는 일련의 작업 단계와 각 단계 간의 전환 조건을 포함합니다.

## 3.7 MessageBus

- 메시지 버스는 서로 다른 컴포넌트, 서비스, 애플리케이션 간에 메시지를 전달하는 시스템입니다. 이는 비동기 통신과 분산 시스템 설계를 지원합니다.

## 3.8 Auth

- 인증(Auth)은 시스템이 사용자의 신원을 확인하는 프로세스입니다. 이는 보통 사용자 이름과 비밀번호를 사용하여 수행됩니다.
